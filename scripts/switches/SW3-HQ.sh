#!/bin/bash
hostnamectl set-hostname SW3-HQ
...
for port in eth0 eth1 eth2; do
  ovs-vsctl add-port SW3-HQ $port
...
ovs-vsctl set port SW3-HQ tag=330
ip addr add 192.168.11.4/24 dev SW3-HQ

# Настройка STP (вторичный коммутатор)
ovs-vsctl set bridge SW3-HQ stp_enable=true
ovs-vsctl set bridge SW3-HQ other_config:stp-priority=32768
